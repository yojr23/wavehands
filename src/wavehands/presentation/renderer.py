...
def draw_status(
    ...
    cv2.putText(frame, f"Nota: {note_label}", (14, 156), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 210, 255), 2)
    chord_label = selected_chord if selected_chord is not None else "--"
    cv2.putText(frame, f"Acorde: {chord_label}", (14, 178), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (235, 190, 110), 2)

    freq_label = f"{frequency_hz:6.2f} Hz" if frequency_hz is not None else "--"
    cv2.putText(frame, f"Frecuencia: {freq_label}", (14, 204), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (245, 245, 245), 1)
